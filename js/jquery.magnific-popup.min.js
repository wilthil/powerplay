/* Portfolio filter functionality */
		...
		jQuery('.portfoliofilter li:first').addClass('active');
			var $filteredData;
			var $filterType = jQuery('.portfolio-filter dd.active a').attr('class');
			var $holder = jQuery('.portfolio-list');
			var $link = jQuery('dl.portfolio-filter dd a');
			var height;

			$link.click(function(e) {
				jQuery('dl.portfolio-filter dd').removeClass('active'); // Remove all active classes
				$filterType = jQuery(this).attr('class');

				jQuery(this).parent().addClass('active');
					if ($filterType === 'showall') {
						$filteredData = jQuery('ul.portfolio-list > li');
					} else {
						$filteredData = jQuery('ul.portfolio-list > li[data-type~=' + $filterType + ']');
					}

				jQuery('ul.portfolio-list > li:visible').fadeOut('500', function(){
					jQuery($filteredData).fadeIn('500');
				});
				return false;
            });
