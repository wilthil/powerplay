...
class sidebar_generator {
	    ...
	    if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				...
				register_sidebar(array(
					'name'=>$sidebar,
					'id' => $sidebar_id,
					'before_widget' => '<aside id="%1$s" class="widget %2$s '.$sidebar_class.'">',
		   			'after_widget' => '</aside>',
	...
	function add_sidebar(){
		...
		$js = "
			var tbl = document.getElementById('veuse_table');
	...
	function admin_menu(){
		add_theme_page( 'Sidebar generator', __('Create sidebars','veuse'), 'manage_options', 'sidebar_generator', array('sidebar_generator', 'admin_page' ));
	}
	...
	function admin_page(){
		...
		<div class="wrap rm_wrap">
			...
			<h2><?php _e('Sidebar Generator','veuse');?></h2>
			
			<div class="rm_section">
			<p><?php _e('Here you can create new sidebars. These sidebars will be selectable in the Sidebars panel in posts and pages','veuse');?></p>
			
			<div class="add_sidebar">
				<a href="javascript:void(0);" onclick="return add_sidebar_link()" title="Add a sidebar" class="button button-primary"><?php _e('+ Add Sidebar','veuse');?></a>
			</div>
			<br />

			
			<table class="widefat page" id="veuse_table">
				<tr>
					<th><?php _e('Sidebars','veuse');?></th>
					<th><?php _e('CSS class','veuse');?></th>
					<th><?php //_e('Delete','veuse');?></th>
				</tr>
				<?php
				$sidebars = sidebar_generator::get_sidebars();
				...
				<tr class="<?php echo $alt?>">
					...
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo $sidebar; ?>',<?php echo $cnt+1; ?>);" title="Remove this sidebar"><?php _e('Delete sidebar','veuse');?></a></td>
				</tr>
				<?php
						$cnt++;
					}
					...
					?>
					<tr>
						<td colspan="3"><?php _e('No Sidebars created','veuse');?></td>
					</tr>
					<?php
				}
				?>
	...
	*/
	function get_sidebar($name="0"){
		...
		$selected_sidebar = get_post_meta($post->ID, 'veuse_selected_sidebar', true);
		$selected_sidebar_replacement = get_post_meta($post->ID, 'veuse_selected_sidebar_replacement', true);
		$did_sidebar = false;
	...
	*/
	function update_sidebars($sidebar_array){
		$sidebars = update_option('veuse_sidebars',$sidebar_array);
	}
	...
	*/
	function get_sidebars(){
		$sidebars = get_option('veuse_sidebars');
		return $sidebars;
	}
